...
<body>
    <?php $excluirUsuario = @$_REQUEST["page"] == "excluir";  ?>
    ...
    <div class="boxLoginContent">
        ...
        <div class="boxFormCadastro">
            ...
            <?php  if(@$_REQUEST["page"] == "editar") : ?>
                <h1>Editar dados</h1>
                <?php endif;
            ?>

            <?php  if(@$_REQUEST["page"] == "excluir") : ?>
                <h1>Tem certeza que deseja excluir esse usuário?</h1>
                <?php endif;
            ?>
            
             <?php  if(@$_REQUEST["page"] == "cadastrar") : ?>
                <h1>Formulário de cadastro</h1>
                <p>Para criar sua conta, preencha o formulário abaixo e aguarde a solicitação ser respondida. Um e-mail será enviado em até 5 dias úteis com as informações do seu acesso.</p>
                <?php endif;
            ?>

            <form action="..\..\Controller\UsuarioController.php" method="post">
                <div>
                    <input type="hidden" name="action" value= <?= @$_REQUEST["page"] ?>>
                    <div class="boxForm__row">
                        <label for="nome">Nome</label>
                        <input placeholder="Ex: Maria Silva" type="text" name="nome" <?= $excluirUsuario ? "disabled" : null ?> id="nome">
                    </div>

                    <div class="boxForm__row">
                        <label for="email">E-mail</label>
                        <input placeholder="Ex: maria.silva@example.com" type="text" <?= $excluirUsuario ? "disabled" : null ?>  name="email" id="email">
                    </div>

                    <div class="boxForm__row">
                        <label for="endereco">Endereço</label>
                        <input placeholder="Ex: Rua das Flores, 123, Apto 45, São Paulo, SP" <?= $excluirUsuario ? "disabled" : null ?>  type="text" name="endereco" id="endereco">
                    </div>

                    <div class="boxForm__row">
                        <label for="telefone">Telefone</label>
                        <input placeholder="Ex: (11) 91234-5678" type="text" <?= $excluirUsuario ? "disabled" : null ?>  name="telefone" id="telefone">
                    </div>

                    <div class="boxForm__row">
                        <label for="curso">Curso</label>
                        <select <?= $excluirUsuario ? "disabled" : null ?>>    
                            <option>Análise e desenvolvimento de sistemas</option>
                            <option>Ciências de dados</option>
                        </select>
                    </div>

                    <div class="boxForm__row">
                        <label for="situacao">Situação</label>
                        <input placeholder="Ex: Estudante, Formado, Em busca de estágio" <?= $excluirUsuario ? "disabled" : null ?>  type="text" name="situacao" id="situacao">
                    </div>

                    <div class="boxForm__row">
                        <label for="cpf">CPF</label>
                        <input placeholder="Ex: 123.456.789-00" type="text" <?= $excluirUsuario ? "disabled" : null ?>  name="cpf" id="cpf">
                    </div>

                    <div class="boxForm__row">
                        <label for="redeSocial">Rede social</label>
                        <input placeholder="Ex: @mariasilva" type="text" <?= $excluirUsuario ? "disabled" : null ?>  name="redeSocial" id="redeSocial">
                    </div>

                    <div class="boxForm__row">
                        <label for="linkedin">Linkedin</label>
                        <input placeholder="Ex: linkedin.com/in/mariasilva" <?= $excluirUsuario ? "disabled" : null ?>  type="text" name="linkedin" id="linkedin">
                    </div>

                    <div class="boxForm__row">
                        <label for="github">Github</label>
                        <input placeholder="Ex: github.com/mariasilva" type="text" <?= $excluirUsuario ? "disabled" : null ?>  name="github" id="github">
                    </div>
                    
                    <?php  if(@$_REQUEST["page"] == "cadastrar") : ?>
                        <div class="boxForm__row">
                            <label for="senha">Senha</label>
                            <input placeholder="Informe uma senha" type="password" <?= $excluirUsuario ? "disabled" : null ?>  name="senha" id="senha">
                        </div>
                        <?php endif;
                    ?>
                    
                    <div class="boxForm__row">
                        <label for="github">Foto do perfil</label>
